// src/dify-provider.ts
import { loadApiKey } from "@ai-sdk/provider-utils";

// src/dify-chat-language-model.ts
import {
  APICallError
...
// src/dify-chat-settings.ts
var defaultLogger = {
  debug: (message, extra) => console.debug(`[dify-ai-provider] ${message}`, extra != null ? extra : ""),
  info: (message, extra) => console.log(`[dify-ai-provider] ${message}`, extra != null ? extra : ""),
  warn: (message, extra) => console.warn(`[dify-ai-provider] ${message}`, extra != null ? extra : ""),
  error: (message, extra) => console.error(`[dify-ai-provider] ${message}`, extra != null ? extra : "")
};

// src/dify-chat-schema.ts
import { z } from "zod";
...
var difyStreamEventBase = z.object({
  event: z.string(),
...
var workflowStartedSchema = difyStreamEventBase.extend({
...
var workflowFinishedSchema = difyStreamEventBase.extend({
...
var nodeStartedSchema = difyStreamEventBase.extend({
...
var nodeFinishedSchema = difyStreamEventBase.extend({
...
var messageSchema = difyStreamEventBase.extend({
  event: z.literal("message"),
  id: z.string().optional(),
  answer: z.string(),
  from_variable_selector: z.array(z.string()).optional()
});
var messageEndSchema = difyStreamEventBase.extend({
...
var ttsMessageSchema = difyStreamEventBase.extend({
  event: z.literal("tts_message"),
  audio: z.string()
});
var ttsMessageEndSchema = difyStreamEventBase.extend({
  event: z.literal("tts_message_end"),
  audio: z.string()
});
var agentMessageSchema = difyStreamEventBase.extend({
  event: z.literal("agent_message"),
  answer: z.string()
});
var agentThoughtSchema = difyStreamEventBase.extend({
...
var messageReplaceSchema = difyStreamEventBase.extend({
  event: z.literal("message_replace"),
  answer: z.string()
});
var messageFileSchema = difyStreamEventBase.extend({
...
var pingSchema = difyStreamEventBase.extend({
  event: z.literal("ping")
});
var errorSchema = difyStreamEventBase.extend({
  event: z.literal("error"),
  status: z.number().optional(),
  code: z.string().optional(),
  message: z.string().optional()
});
var difyStreamEventSchema = z.discriminatedUnion("event", [
  ...
  pingSchema,
  errorSchema
]).or(difyStreamEventBase);

// src/stream-parser.ts
var ThinkTagParser = class {
  constructor() {
    this.state = "text";
    this.buffer = "";
    this.accumulated = "";
    ...
    this.closed = false;
    // Once </think> is seen, treat all subsequent <think> as plain text
    this.textAccumulated = "";
    this.inString = false;
    this.escape = false;
  }
  feed(chunk) {
  ...
  flush() {
    ...
    const seg = [{ type, content: this.buffer }];
    if (type === "text") {
...
function parseToolCalls(text, toolNames, genId) {
  const toolCalls = [];
  let cleanText = text;
...
function placeholder(schema) {
  ...
  if (t === "object") {
    ...
    if (!props) return "{}";
    const entries = Object.entries(props).slice(0, 3).map(([k, v]) => `"${k}": ${placeholder(v)}`);
    return `{${entries.join(", ")}}`;
  }
...
function formatToolsPrompt(tools) {
  ...
  const defs = tools.filter((t) => t.type === "function").map((t) => ({ name: t.name, description: t.description, parameters: t.inputSchema }));
  if (!defs.length) return { prompt: "", toolNames: [] };
  const toolNames = defs.map((d) => d.name);
  const toolDescriptions = defs.map((t) => {
    const params = t.parameters;
    const paramList = (params == null ? void 0 : params.properties) ? Object.entries(params.properties).map(([k, v]) => `"${k}": ${placeholder(v)}`).join(", ") : "";
...
async function uploadFileToDify(data, mimeType, filename, user, baseURL, headers, logger) {
  try {
    let blob;
    ...
    if (!res.ok) {
      ...
      logger == null ? void 0 : logger.error("Dify file upload failed", { status: res.status, statusText: res.statusText, body: errBody });
      return void 0;
    }
    const json = await res.json();
    return json.id;
  } catch (e) {
    logger == null ? void 0 : logger.error("Dify file upload error", { error: String(e) });
    return void 0;
  }
}
async function extractFileAttachments(messages, userId, baseURL, headers, logger) {
  ...
  for (const msg of messages) {
    ...
    for (const part of msg.content) {
      ...
      if (uploadData) {
        const task = uploadFileToDify(uploadData, mimeType, filename, userId, baseURL, headers, logger).then((uploadId) => {
          if (uploadId) {
            return {
            ...
            logger == null ? void 0 : logger.warn("Dify file upload skipped, attachment will be missing", { filename, mimeType });
            return void 0;
          }
        });
...
// src/dify-chat-language-model.ts
function formatSystemPrompt(raw) {
  if (!raw) return "";
...
function buildRetrieverResourceSources(resources, metadata) {
  if (!Array.isArray(resources)) return [];
  return resources.map((resource, index) => {
    var _a, _b;
    const record = isRecord(resource) ? resource : {};
    const id = (_a = getStringField(record, ["segment_id", "document_id", "id"])) != null ? _a : `retriever-resource-${index + 1}`;
    const title = (_b = getStringField(record, ["document_name", "title", "dataset_name", "segment_id", "id"])) != null ? _b : `Retriever resource ${index + 1}`;
...
var DifyChatLanguageModel = class {
  constructor(modelId, settings, config) {
    ...
    this.logger = settings.logger === false ? void 0 : (_a = settings.logger) != null ? _a : defaultLogger;
  ...
  createErrorHandler() {
    return createJsonErrorResponseHandler({
      ...
      errorToMessage: (data) => {
        var _a;
        (_a = this.logger) == null ? void 0 : _a.error("Dify API error", data);
  ...
  async doGenerate(options) {
    ...
    if (this.settings.logMessages) {
      (_a = this.logger) == null ? void 0 : _a.info("", { body });
    }
    const { responseHeaders, value: data } = await postJsonToApi({
      url: this.chatMessagesEndpoint,
      headers: combineHeaders(this.config.headers(), requestHeaders != null ? requestHeaders : options.headers),
      body,
      abortSignal,
    ...
    if (this.settings.logMessages) {
      (_b = this.logger) == null ? void 0 : _b.info("", { answer: typedData.answer, conversation_id: typedData.conversation_id });
    }
    const content = [];
    if (typedData.answer) {
  ...
  async doStream(options) {
    var _a, _b;
    if (this.settings.logMessages) {
      (_a = this.logger) == null ? void 0 : _a.info("", { options });
    }
    const { abortSignal } = options;
    const { body: requestBody, toolNames, requestHeaders } = await this.getRequestBody(options);
    const body = { ...requestBody, response_mode: "streaming" };
    if (this.settings.logMessages) {
      (_b = this.logger) == null ? void 0 : _b.info("", { body });
    }
    ...
    function emitSegments(segments, controller) {
      ...
      stream: responseStream.pipeThrough(
        new TransformStream({
          transform(chunk, controller) {
            ...
            if (!chunk.success) {
              logger == null ? void 0 : logger.error("Dify stream parse error", { error: String(chunk.error) });
              controller.enqueue({ type: "error", error: chunk.error });
              return;
            }
            const data = chunk.value;
            if (logMessages) {
              logger == null ? void 0 : logger.info("", { data });
            }
            ...
            switch (data.event) {
              ...
              case "message_end": {
                ...
                if (data.event === "workflow_finished") {
                  const dataUsage = data.data || {};
                  inputTokens = 0;
                  outputTokens = (_a2 = dataUsage.total_tokens) != null ? _a2 : 0;
                  totalTokens = (_b2 = dataUsage.total_tokens) != null ? _b2 : 0;
                } else {
                  const usageData = ((_c = data.metadata) == null ? void 0 : _c.usage) || {};
                  inputTokens = (_d = usageData.prompt_tokens) != null ? _d : 0;
                  outputTokens = (_e = usageData.completion_tokens) != null ? _e : 0;
                  totalTokens = (_f = usageData.total_tokens) != null ? _f : 0;
                }
                ...
                if (logMessages) {
                  logger == null ? void 0 : logger.info("", { toolCalls });
                }
                ...
                if (data.event === "message_end") {
                  for (const sourcePart of buildRetrieverResourceSources(
                    (_g = data.metadata) == null ? void 0 : _g.retriever_resources,
              ...
              case "error": {
                const err = data;
                const msg = (_i = (_h = err.message) != null ? _h : err.msg) != null ? _i : "Unknown Dify error";
                const code = (_k = (_j = err.code) != null ? _j : err.status) != null ? _k : "";
                logger == null ? void 0 : logger.error("Dify stream error", { code, message: msg });
   ...
   */
  async getRequestBody(options) {
    var _a, _b, _c, _d, _e;
    const opts = options;
    const messages = (_a = options.prompt) != null ? _a : opts.messages;
    if (!messages || !messages.length) {
      (_b = this.logger) == null ? void 0 : _b.error("No messages provided", { prompt: options.prompt });
    ...
    if (!query) {
      (_c = this.logger) == null ? void 0 : _c.error("No user message found", { messageCount: messages.length, roles: messages.map((m) => m.role) });
      throw new APICallError({
    ...
    const userId = (_e = (_d = options.headers) == null ? void 0 : _d["user-id"]) != null ? _e : "you_should_pass_user-id";
  ...
  findConversationId(headers, messages) {
    var _a, _b;
    if (headers == null ? void 0 : headers["chat-id"]) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (Array.isArray(msg.content)) {
        for (const part of msg.content) {
          const meta = ((_a = part == null ? void 0 : part.providerMetadata) == null ? void 0 : _a.difyWorkflowData) || ((_b = part == null ? void 0 : part.callProviderMetadata) == null ? void 0 : _b.difyWorkflowData);
          if (meta == null ? void 0 : meta.conversationId) {
  ...
  buildQuery(messages, conversationId) {
    let startIndex = 0;
    if (conversationId) {
    ...
    return messages.slice(startIndex).filter((msg) => msg.role === "user" || msg.role === "tool" || !conversationId && msg.role === "assistant").map((msg) => {
  ...
  extractTextFromMessage(msg) {
    ...
    if (Array.isArray(msg.content)) {
      return msg.content.map((part) => {
        ...
        if ((part == null ? void 0 : part.type) === "text" && part.text) return part.text;
        return "";
      }).filter(Boolean).join(" ");
    }
    return "";
  }
  extractToolResult(msg) {
    var _a, _b, _c;
    ...
    for (const part of content) {
      if ((part == null ? void 0 : part.type) === "tool-result") {
        ...
        const raw = (_c = (_b = (_a = part.output) != null ? _a : part.result) != null ? _b : part.content) != null ? _c : part.text;
        const output = raw !== void 0 && typeof raw === "object" && raw !== null && "value" in raw ? raw.value : raw;
        const result = output === void 0 ? "[completed]" : typeof output === "string" ? output : JSON.stringify(output);
...
// src/dify-provider.ts
function createDifyProvider(options = {}) {
  const createChatModel = (modelId, settings = {}) => new DifyChatLanguageModel(modelId, settings, {
    provider: "dify.chat",
...
var difyProvider = createDifyProvider();
export {
  createDifyProvider,
  difyProvider
};
